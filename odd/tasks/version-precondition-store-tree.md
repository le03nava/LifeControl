...
| W2-T5 | Work-unit commit + PR | evidence log row | done — commit `5345a5b` and the 412 follow-up `3c56028`; PR #184, merged as `ca873f7` |
| **W3** | **Slice 3 — `store-zones`** | | |
| W3-T1 | Tests: the same five integration behaviours, the controller 412 envelope **plus the duplicate-409 guard**, and Angular specs for the payload merge, the create key's absence, `version: 0`, the 412 recovery and the 409 fall-through | `StoreZoneVersionPreconditionIntegrationTest` (5 tests), `StoreZoneControllerTest`, `store-zones-edit.spec.ts` | done — the RED was measured retroactively by reverting the fix, see the evidence log |
| W3-T2 | Backend: `UpdateStoreZoneRequest`, `StoreZoneResponse`, `StoreZoneService` (assert + 3 flushes + message constant) | 3 src files + 3 controller-test constructor sites + `StoreZoneServiceTest`'s stub/verify rename | done |
| W3-T3 | Angular: `store-zone.models.ts`, `store-zones-edit.ts` and every `StoreZone` fixture the compiler forces | 5 spec files, fixtures only | done |
| W3-T4 | Gates green on the committed tree | evidence log rows | done — 612 suites / 2148 tests / 0 failures; 130 files / 2553 tests / 0 failures |
| W3-T5 | Work-unit commit + PR | evidence log row | done — commits `476a3b6` and `22ccdbb`; PR #185, merged as `f2ca839` |
| **W4** | **Slice 4 — `store-locations`** | | |
| W4-T1 | Tests: the same five integration behaviours, the controller 412 envelope **plus the duplicate-409 guard**, and Angular specs for the payload merge, the create key's absence, `version: 0`, the 412 recovery and the 409 fall-through | `StoreLocationVersionPreconditionIntegrationTest` (5 tests), `StoreLocationControllerTest`, `store-locations-edit.spec.ts` | done — the RED was measured retroactively by reverting the fix, see the evidence log |
...
| 2026-09-26 | **W2 delivered: PR #184 merged as `ca873f7`** (parents `50826e0` + `3c56028`), 17 files / +688 −63, branch and worktree removed. The tree-identity check applies to this delivery and holds, because the W2 branch had merged `main` before it was delivered: `main^{tree}` == `3c56028^{tree}` == `b9721d564f9aa65d96485b22d8e9df282d45ebe3`, and the file-level check agrees (`git diff --name-only 3c56028 main` → 0 files). Post-merge CI on `main` @ `ca873f7`: **4/4 green** (API CI, Angular CI, Gateway CI, Docker Build Integrity, all `success`); the same four are green on `50826e0` after W1 | `gh pr view 184`; `git rev-parse` on both trees; `gh run list --branch main` |
| 2026-09-26 | **W3 committed as `476a3b6`** (`fix(store-zones): enforce the version precondition with its own 412`), 15 files / +672 −43: 3 backend src files, 4 backend test files (one new), the Angular model, the edit page, its spec and 4 forced fixtures. The pre-commit hook ran `eslint --fix` + `prettier --write` over the 7 staged `.ts` files and **changed no byte**: `sha256sum -c` over all 15 files, taken before the gate run, still reports `OK` after the commit, so the gate evidence below belongs to exactly the committed tree | `git log -1 --stat`; the sha256 before/after comparison |
| 2026-09-26 | **W3 gates green on those bytes.** Backend `./gradlew spotlessCheck spotbugsMain cleanTest test --no-daemon` → BUILD SUCCESSFUL; aggregated JUnit XML **612 suites / 2148 tests / 0 failures / 0 errors / 0 skipped**. Frontend `lint` exit 0, `build` exit 0, `test:coverage:check` → **130 files / 2553 tests / 0 failures / 0 errors / 0 skipped**, coverage **94.09 / 75.96 / 89.34 / 94.09** against the 80 / 60 / 75 / 80 thresholds | the five runs; the XML under `life-control-api/build/test-results/test/` and `coverage/coverage-summary.json` |
| 2026-09-26 | **W3 independent read-only verification round** (delegated, over the uncommitted diff): **0 blocking findings**; all nine criteria met except criterion 9, which is not satisfiable before the commit and is proven by the sha256 row above; contract items 1–8 met and confirmed by construction for the additive response; the assert ordering verified against every mutation path; `deleteZone`/`disableZone`/`disableZonesOfAreas` confirmed still on `save(...)`; no other slice touched. It also confirmed that the `history.state` seed cannot survive the authoritative lookup (it never seeds `chain`, so a save cannot run on a stale version) and that no constructor/test change weakened an existing assertion — one was **strengthened**. Non-blocking, recorded rather than fixed: two of the five new Angular specs (the create-body key absence and the 409 no-reload) pass on the reverted base, so they are regression guards and not RED discriminators — the same nuance W2 recorded | the verification report; `store-zones-edit.spec.ts`; `StoreZoneService.java` |
| 2026-09-26 | **W3 RED reconstructed, not observed in order.** Backend, with `StoreZoneResponse` + `StoreZoneService` + the 4 controller/service tests at their `main` state (keeping the new request DTO and the new integration test, the minimum that compiles) → **147 tests completed, 4 failed** (`staleVersionPutAnswers412AndWritesNothing`, `currentVersionPutAnswers200WithIncrementedVersionAndFreshUpdatedAt`, `enablePatchAnswersPostFlushVersionAndFreshUpdatedAt`, `postAnswersTimestampsAndVersion`); the absent-version pass-through correctly stays green because it pins the old contract. Frontend, with only `store-zones-edit.ts` reverted → **4 failed / 47 passed of 51**, and one of the four is a **pre-existing** spec the change makes stricter (`should update using the chain from the response`). The page was restored byte-identically (`sha256sum` equal before and after); the backend files were restored deterministically and proven by the green gate re-run, since no pre-revert snapshot of them was taken — stated as the limitation it is | the two runs; the backend restore proof |
| 2026-09-26 | **Tooling note for the remaining slices:** `npm ci` **fails** in a fresh slice worktree on a pre-existing `zone.js` peer conflict (`zone.js@^0.16.2` in `package.json` vs `peerOptional zone.js@~0.15.0` from `@angular/core`). `npm ci --legacy-peer-deps` succeeds, which is what `angular-ci.yml` itself runs. A linked worktree also starts with no `node_modules` and no `.husky/_`, so `npm ci --legacy-peer-deps` is the first step in any new slice worktree — otherwise the pre-commit hook silently does not run | `/tmp` install log; `.github/workflows/angular-ci.yml:36` |
| 2026-09-26 | **W1's post-merge CI closed**: the two runs *that slice's own delivery row* recorded as still in flight finished green — API CI and Angular CI, both `success` on `50826e0`, together with Gateway CI and Docker Build Integrity. W1 therefore has the same 4/4 post-merge evidence the two later slices have. Recorded as a later row rather than by editing the earlier one, so the difference between what was known then and what is known now stays visible | `gh run list --branch main` |
| 2026-09-26 | **W3 delivered: PR #185 merged as `f2ca839`** (parents `ca873f7` + `22ccdbb`), 16 files / +691 −49, branch and worktree removed. The tree-identity check applies here and holds, because the branch contained everything `main` had when it was delivered: `main^{tree}` == `22ccdbb^{tree}` == `7756f8dbbc836beba8a359d19305e943a2a04493`, and the file-level check agrees (`git diff --name-only main 22ccdbb` → 0 files). Both branch commits are ancestors of `main`. Post-merge CI on `main` @ `f2ca839`: **4/4 green** (API CI, Angular CI, Gateway CI, Docker Build Integrity). Merge order as the worktree reference prescribes: the herdr workspace was closed **before** the merge, so `gh pr merge --delete-branch` removed the linked worktree instead of leaving a hung workspace. Final clone state: only the anchor worktree, only `main` locally, `origin` with `main` alone, tree clean | `gh pr view 185`; `git rev-parse` on both trees; `git worktree list`; `git ls-remote --heads origin`; `gh run list --branch main` |
| 2026-09-26 | **The registry's header no longer describes the slice in flight** (PR #186 merged as `d73cc5f`, 1 file / +6 −5). That row set the rule this slice follows: a header statement about the current slice's pull request **is falsified by its own merge**, silently, with no commit touching the lying sentence — it happened to W2 and then to W3. From W4 on, the header states only durable facts per slice (delivered + merge) and names the remaining **task** (W5), which stays true before and after the merge | this record's header at `0cc6913`; `d73cc5f` |
| 2026-09-26 | **W4 committed as `0cc6913`** (`fix(store-locations): enforce the version precondition with its own 412`), 14 files / +755 −47: 3 backend src files, 5 backend test files (one new), the Angular model, the edit page, its spec and 4 forced fixtures. The pre-commit hook ran `eslint --fix` + `prettier --write` and **changed no byte**: `sha256sum -c` over all 14 files, taken before the gate run, still reports `OK` after the commit, so the gate evidence below belongs to exactly the committed tree | `git log -1 --stat`; the sha256 before/after comparison |
| 2026-09-26 | **W4 gates green on those bytes.** Backend `./gradlew spotlessCheck spotbugsMain cleanTest test --no-daemon` → BUILD SUCCESSFUL; aggregated JUnit XML **613 suites / 2154 tests / 0 failures / 0 errors / 0 skipped**. Frontend `lint` exit 0, `build` exit 0, `test:coverage:check` exit 0 → **130 files / 2558 tests / 0 failures / 0 errors / 0 skipped**, coverage **94.10 / 75.95 / 89.35 / 94.10** against the 80 / 60 / 75 / 80 thresholds | the five runs; the XML under `life-control-api/build/test-results/test/` and `coverage/coverage-summary.json` |
