...
| Retarget | **`gh api -X PATCH repos/le03nava/LifeControl/pulls/<n> -f base=main`** used for #120 and #121 — `gh pr edit <n> --base` fails on this machine. After each retarget the diff was re-checked and stayed exactly the slice's own size, because the merged parent commit is already an ancestor of `main`, so **no rebase was needed** |
