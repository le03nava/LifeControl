...
**Status**: **merged to `main`** @ `b355d7e` on 2026-09-25 — the four slices landed bottom-up, one pull
request each, after each slice's own independent verification (see the task log): **#173**
(`feat/w3-slice-1-movement-engine`, merge `5995771`), **#174** (`feat/w3-slice-2-sales-path`, merge
`583426c`), **#175** (`feat/w3-slice-3-balance-reset`, merge `0dcef7a`) and **#176**
(`feat/w3-slice-4-contract-flip`, merge `b355d7e`). API CI was green on every head before its merge and on
the merged tree; the slice branches are deleted.
**No slice work remains open.** What remains are the six follow-ups this feature opened, `F-1`…`F-6`
under `### Follow-ups opened by slice 2`: deferred by decision, not blocked.
...
| 2026-09-25 | Delivery completed: the four stacked pull requests merged bottom-up | Merge order was the maintainer's decision and the answer was `1 → 2 → 3 → 4`, so each merge delivered exactly its own slice and no more: **#173** (`feat/w3-slice-1-movement-engine` @ `5995771`), **#174** (`feat/w3-slice-2-sales-path` @ `583426c`), **#175** (`feat/w3-slice-3-balance-reset` @ `0dcef7a`), **#176** (`feat/w3-slice-4-contract-flip` @ `b355d7e`), each as a merge commit carrying its branch deletion, after the record had left merge order open as the maintainer's decision. The provenance branch `feat/sales-location-aware-stock` and its worktree were left alone. Every head's API CI job was green **before** its merge (`3m15s`, `3m38s`, `3m16s`, `3m20s`), and after the fourth merge API CI ran on `main` itself (run `36198094442`) and passed. **The three push runs on `main` before that one report `cancelled`** — nothing to report as a failure and no silence either: it is this workflow's own `concurrency: api-ci-${{ github.ref }}` with `cancel-in-progress: true` superseding them on the same ref, so the run on the merged tree is the only push gate that finished. Ancestry checked against `origin/main` rather than assumed: `ac12904` (slice 1), `37ec810` (slice 2), `1ade62a` (slice 3) and `a6badfe` (slice 4) are all ancestors of it, and the merged tree carries `V15__inventory_balance_reset.sql` and the five `MovementType` constants (`RECEIPT`, `SALE`, `SALE_REVERSAL`, `ADJUSTMENT_INCREASE`, `ADJUSTMENT_DECREASE`). **What gated these merges is worth stating plainly:** `main` had no branch protection and no ruleset at the time (`Branch not protected` / `rulesets: []`), so the four CI jobs were the only gate — nothing enforced the order chosen here, and the order is what kept each merge slice-sized |
