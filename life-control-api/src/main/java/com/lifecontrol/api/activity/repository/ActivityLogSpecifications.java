...
public final class ActivityLogSpecifications {
    ...
    public static Specification<ActivityLog> withFilters(LocalDateTime from,
                                                        LocalDateTime to,
                                                        String process,
                                                        String event,
                                                        String userId,
        ...
        return (root, query, cb) -> {
            ...
            if (process != null) {
                predicates.add(cb.equal(root.join("activityProcess", JoinType.INNER).get("name"), process));
            }
            if (event != null) {
                predicates.add(cb.equal(root.join("activityEvent", JoinType.INNER).get("name"), event));
            }
            if (userId != null) {
