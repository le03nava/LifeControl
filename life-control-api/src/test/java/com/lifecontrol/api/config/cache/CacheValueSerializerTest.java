...
class CacheValueSerializerTest {
    ...
    void roundTrip_CountryResponse() {
        var country = new CountryResponse(
                UUID.randomUUID(), "MX", "México", true,
                LocalDateTime.of(2026, 9, 14, 10, 30), LocalDateTime.of(2026, 9, 14, 11, 0));
    ...
    void roundTrip_ImmutableListOfRecords() {
        var countries = List.of(
                ...
                new CountryResponse(UUID.randomUUID(), "US", "United States", false, LocalDateTime.now(), LocalDateTime.now()));

        var restored = roundTrip(countries);

        assertThat(restored).asInstanceOf(org.assertj.core.api.InstanceOfAssertFactories.LIST)
    ...
    void roundTrip_ArrayListOfRecords() {
        var statuses = Arrays.asList(
                new StatusResponse(UUID.randomUUID(), "Draft", UUID.randomUUID(), "SALES_ORDER", true, LocalDateTime.now(), LocalDateTime.now()));
    ...
    void roundTrip_OtherCachedRecords() {
        var measureUnit = new MeasureUnitResponse(
                UUID.randomUUID(), "Kilogram", "kg", "WEIGHT", "KGM", "Peso", true,
                LocalDateTime.now(), LocalDateTime.now());
        var zone = new CompanyZoneResponse(
                UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(),
                "Z-01", "Centro", "desc", 1, true, LocalDateTime.now(), LocalDateTime.now());
