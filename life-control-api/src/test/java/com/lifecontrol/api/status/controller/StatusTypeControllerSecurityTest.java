...
class StatusTypeControllerSecurityTest {
    ...
    private StatusTypeRequest buildRequest() {
        return new StatusTypeRequest("Order Status", true);
    }
    ...
    class CreateStatusTypeSecurity {
        ...
        @DisplayName("returns 201 Created for user with lc-status-type role")
    ...
    class UpdateStatusTypeSecurity {

        @Test
        @WithMockUser(roles = {"lc-admin"})
        @DisplayName("returns 200 OK for user with lc-admin role")
        void adminCanUpdate() throws Exception {
            when(statusTypeService.updateStatusType(eq(statusTypeId), any(StatusTypeRequest.class)))
                    .thenReturn(buildResponse());

            mockMvc.perform(put("/api/status-types/{id}", statusTypeId)
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(buildRequest())))
                    .andExpect(status().isOk());
        }
        ...
        @Test
        @WithMockUser(roles = {"lc-status-type"})
        @DisplayName("returns 200 OK for user with lc-status-type role")
    ...
    class DeleteStatusTypeSecurity {
