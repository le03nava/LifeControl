...
class StatusControllerSecurityTest {
    ...
    private StatusResponse buildResponse() {
        return new StatusResponse(
                statusId, "ACTIVO", statusTypeId, "Order Status", true, LocalDateTime.now(), LocalDateTime.now());
    }
    ...
    class GetStatusesSecurity {
        ...
        void anyAuthenticatedUserCanRead() throws Exception {
            ...
            mockMvc.perform(get("/api/statuses").param("statusTypeId", statusTypeId.toString()))
    ...
    class CreateStatusSecurity {
        ...
        void adminCanCreate() throws Exception {
            when(statusService.createStatus(any(StatusRequest.class))).thenReturn(buildResponse());
    ...
    class DeleteStatusSecurity {
        ...
        void adminCanDelete() throws Exception {
            mockMvc.perform(delete("/api/statuses/{id}", statusId)).andExpect(status().isNoContent());
        }
        ...
        @DisplayName("returns 403 Forbidden for authenticated user with no roles")
        void userWithNoRolesGetsForbidden() throws Exception {
            mockMvc.perform(delete("/api/statuses/{id}", statusId)).andExpect(status().isForbidden());
        }
    }
    ...
    class EnableStatusSecurity {
        ...
        void adminCanEnable() throws Exception {
            when(statusService.enableStatus(statusId)).thenReturn(buildResponse());

            mockMvc.perform(patch("/api/statuses/{id}/enable", statusId)).andExpect(status().isOk());
        }
        ...
        @DisplayName("returns 403 Forbidden for user with wrong role")
        void userWithWrongRoleGetsForbidden() throws Exception {
            mockMvc.perform(patch("/api/statuses/{id}/enable", statusId)).andExpect(status().isForbidden());
        }
    }
}
