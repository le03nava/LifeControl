...
/**
 * HTTP-level verification of the {@code store_zones} version precondition and of the post-flush
...
@DisplayName("Store zone version precondition Integration Tests")
class StoreZoneVersionPreconditionIntegrationTest extends AbstractPostgresIntegrationTest {
    ...
    private static final String ZONE_URL = BASE_URL + "/{storeZoneId}";
    private static final String ENABLE_URL = ZONE_URL + "/enable";
     ...
     * &rarr; store-zone chain.
     */
    private void seedCompanyHierarchy() {
        var country = countryRepository.findByCountryCode("MX").orElseThrow();

        var company = companyRepository
                .findByCompanyKey("STORE-ZONE-VERSION-KEY")
                .orElseGet(() -> companyRepository.save(Company.builder()
                        .companyKey("STORE-ZONE-VERSION-KEY")
                        .companyName("Store Zone Version Test Company")
                        .rfc("SZVR010101ABC")
                        .enabled(true)
                        .build()));
        ...
        var region = companyRegionRepository.findByCompanyCountryIdOrderByRegionNameAsc(companyCountry.getId()).stream()
                ...
                .orElseGet(() -> companyRegionRepository.save(CompanyRegion.builder()
                        ...
                        .regionCode("SZV")
                        .regionName("Store Zone Version Region")
                        .enabled(true)
                        .build()));
        ...
        var zone = companyZoneRepository.findByCompanyRegionIdOrderByZoneNameAsc(region.getId()).stream()
                ...
                .orElseGet(() -> companyZoneRepository.save(CompanyZone.builder()
                        ...
                        .zoneCode("SZV")
                        .zoneName("Store Zone Version Zone")
                        .enabled(true)
                        .build()));
        ...
        var store = companyStoreRepository.findByCompanyZoneId(zone.getId()).stream()
                ...
                .orElseGet(() -> companyStoreRepository.save(CompanyStore.builder()
                        .companyZone(zone)
                        .storeName("Store Zone Version Store")
                        .enabled(true)
                        .build()));
    ...
    private ResultActions putZone(UpdateStoreZoneRequest request) throws Exception {
        return mockMvc.perform(
                put(ZONE_URL, companyId, companyCountryId, regionId, zoneId, storeId, areaId, storeZoneId)
                        .with(jwt().authorities(ROLE_LC_ADMIN))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)));
    }

    private ResultActions getZone() throws Exception {
        return mockMvc.perform(
                get(ZONE_URL, companyId, companyCountryId, regionId, zoneId, storeId, areaId, storeZoneId)
                        .with(jwt().authorities(ROLE_LC_ADMIN)));
    }
    ...
    void staleVersionPutAnswers412AndWritesNothing() throws Exception {
        ...
        long versionBefore =
                storeZoneRepository.findById(storeZoneId).orElseThrow().getVersion();
        ...
        putZone(new UpdateStoreZoneRequest(null, acceptedName, null, null)).andExpect(status().isOk());

        var accepted = storeZoneRepository.findById(storeZoneId).orElseThrow();
        ...
        // The same zone named again with an outdated version must be rejected.
        putZone(new UpdateStoreZoneRequest(null, "Cambio perdido", null, null, acceptedVersion - 1))
                .andExpect(status().isPreconditionFailed())
                .andExpect(jsonPath("$.status").value(412));

        var after = storeZoneRepository.findById(storeZoneId).orElseThrow();
        assertThat(after.getZoneName()).isEqualTo(acceptedName);
    ...
    void currentVersionPutAnswers200WithIncrementedVersionAndFreshUpdatedAt() throws Exception {
        ...
        var responseBody = putZone(new UpdateStoreZoneRequest(null, acceptedName, null, null, versionBefore))
                .andExpect(status().isOk())
                .andReturn()
                .getResponse()
                .getContentAsString();
        ...
        var stored = storeZoneRepository.findById(storeZoneId).orElseThrow();
        assertThat(stored.getVersion()).isEqualTo(versionBefore + 1);
        assertThat(stored.getZoneName()).isEqualTo(acceptedName);
    }
    ...
    void putWithoutVersionAnswers200() throws Exception {
        ...
        putZone(new UpdateStoreZoneRequest(null, "Sin versión " + UUID.randomUUID(), null, null))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.zoneName").exists());

        assertThat(storeZoneRepository.findById(storeZoneId).orElseThrow().getZoneName())
    ...
    void enablePatchAnswersPostFlushVersionAndFreshUpdatedAt() throws Exception {
        // The enable endpoint takes no body, so the zone is soft-deleted through the real DELETE
        // endpoint first.
        mockMvc.perform(delete(ZONE_URL, companyId, companyCountryId, regionId, zoneId, storeId, areaId, storeZoneId)
                        .with(jwt().authorities(ROLE_LC_ADMIN)))
                .andExpect(status().isNoContent());

        var disabled = storeZoneRepository.findById(storeZoneId).orElseThrow();
        long versionBefore = disabled.getVersion();
        LocalDateTime updatedAtBefore = disabled.getUpdatedAt();

        var body = mockMvc.perform(
                        patch(ENABLE_URL, companyId, companyCountryId, regionId, zoneId, storeId, areaId, storeZoneId)
                                .with(jwt().authorities(ROLE_LC_ADMIN)))
                .andExpect(status().isOk())
                .andReturn()
                .getResponse()
    ...
    void postAnswersTimestampsAndVersion() throws Exception {
        // The area's zone code is unique per area, and the row outlives the method, so the new code
        // must differ run to run.
        String uniqueCode = "V" + UUID.randomUUID().toString().replace("-", "").substring(0, 9);
        var body = mockMvc.perform(post(BASE_URL, companyId, companyCountryId, regionId, zoneId, storeId, areaId)
                        .with(jwt().authorities(ROLE_LC_ADMIN))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(
                                new CreateStoreZoneRequest(uniqueCode, "Zona Nueva Con Versión", null, null))))
