...
/**
 * HTTP-level verification of the {@code store_locations} version precondition and of the
 * post-flush response mapping, against real PostgreSQL with Flyway enabled.
 *
 ...
 * version still passes. The version increment and the fresh timestamp are exactly what the {@code
 * saveAndFlush} change buys: Hibernate advances {@code @Version} and runs the {@code Auditable}
 * callbacks at flush time, so a response mapped from a non-flushed entity would be stale on
 * both.</p>
 */
@SpringBootTest
@AutoConfigureMockMvc
@DisplayName("Store location version precondition Integration Tests")
class StoreLocationVersionPreconditionIntegrationTest extends AbstractPostgresIntegrationTest {
    ...
    private static final String LOCATION_URL = BASE_URL + "/{storeLocationId}";
    private static final String ENABLE_URL = LOCATION_URL + "/enable";
    ...
    /**
     * Find-or-create the company &rarr; country &rarr; region &rarr; zone &rarr; store &rarr; area
     * &rarr; store-zone &rarr; store-location chain.
     */
    private void seedCompanyHierarchy() {
        var country = countryRepository.findByCountryCode("MX").orElseThrow();

        var company = companyRepository
                .findByCompanyKey("STORE-LOCATION-VERSION-KEY")
                .orElseGet(() -> companyRepository.save(Company.builder()
                        .companyKey("STORE-LOCATION-VERSION-KEY")
                        .companyName("Store Location Version Test Company")
                        .rfc("SLVR010101ABC")
                        .enabled(true)
                        .build()));
        ...
        var region = companyRegionRepository.findByCompanyCountryIdOrderByRegionNameAsc(companyCountry.getId()).stream()
                ...
                .orElseGet(() -> companyRegionRepository.save(CompanyRegion.builder()
                        ...
                        .regionCode("SLV")
                        .regionName("Store Location Version Region")
                        .enabled(true)
                        .build()));
        ...
        var zone = companyZoneRepository.findByCompanyRegionIdOrderByZoneNameAsc(region.getId()).stream()
                ...
                .orElseGet(() -> companyZoneRepository.save(CompanyZone.builder()
                        ...
                        .zoneCode("SLV")
                        .zoneName("Store Location Version Zone")
                        .enabled(true)
                        .build()));
        ...
        var store = companyStoreRepository.findByCompanyZoneId(zone.getId()).stream()
                ...
                .orElseGet(() -> companyStoreRepository.save(CompanyStore.builder()
                        .companyZone(zone)
                        .storeName("Store Location Version Store")
                        .enabled(true)
                        .build()));
        ...
        var storeZone = storeZoneRepository.findByStoreAreaIdOrderByDisplayOrderAscZoneCodeAsc(area.getId()).stream()
                .filter(candidate -> "VER".equals(candidate.getZoneCode()))
                .findFirst()
                .orElseGet(() -> storeZoneRepository.save(StoreZone.builder()
                        .storeArea(area)
                        .zoneCode("VER")
                        .zoneName("Version Precondition Zone")
                        .displayOrder(1)
                        .enabled(true)
                        .build()));
        ...
        var storeLocation =
                storeLocationRepository
                        .findByStoreZoneIdOrderByDisplayOrderAscLocationCodeAsc(storeZone.getId())
                        .stream()
                        .filter(candidate -> "VER".equals(candidate.getLocationCode()))
                        .findFirst()
                        .orElseGet(() -> storeLocationRepository.save(StoreLocation.builder()
                                .storeZone(storeZone)
                                .locationCode("VER")
                                .locationName("Version Precondition Location")
                                .displayOrder(1)
                                .enabled(true)
                                .build()));
    ...
    private ResultActions putLocation(UpdateStoreLocationRequest request) throws Exception {
    ...
    private ResultActions getLocation() throws Exception {
    ...
    void staleVersionPutAnswers412AndWritesNothing() throws Exception {
        ...
        long versionBefore =
                storeLocationRepository.findById(storeLocationId).orElseThrow().getVersion();
        ...
        putLocation(new UpdateStoreLocationRequest(null, acceptedName, null, null))
                .andExpect(status().isOk());

        var accepted = storeLocationRepository.findById(storeLocationId).orElseThrow();
        ...
        // The same location named again with an outdated version must be rejected.
        putLocation(new UpdateStoreLocationRequest(null, "Cambio perdido", null, null, acceptedVersion - 1))
                .andExpect(status().isPreconditionFailed())
                .andExpect(jsonPath("$.status").value(412));

        var after = storeLocationRepository.findById(storeLocationId).orElseThrow();
        assertThat(after.getLocationName()).isEqualTo(acceptedName);
    ...
    void currentVersionPutAnswers200WithIncrementedVersionAndFreshUpdatedAt() throws Exception {
        ...
        var responseBody = putLocation(new UpdateStoreLocationRequest(null, acceptedName, null, null, versionBefore))
                .andExpect(status().isOk())
                .andReturn()
                .getResponse()
                .getContentAsString();
        ...
        var stored = storeLocationRepository.findById(storeLocationId).orElseThrow();
        assertThat(stored.getVersion()).isEqualTo(versionBefore + 1);
        assertThat(stored.getLocationName()).isEqualTo(acceptedName);
    }
    ...
    void putWithoutVersionAnswers200() throws Exception {
        ...
        putLocation(new UpdateStoreLocationRequest(null, "Sin versión " + UUID.randomUUID(), null, null))
                .andExpect(status().isOk())
    ...
    void enablePatchAnswersPostFlushVersionAndFreshUpdatedAt() throws Exception {
        // The enable endpoint takes no body, so the location is soft-deleted through the real DELETE
        // endpoint first.
        ...
        var disabled = storeLocationRepository.findById(storeLocationId).orElseThrow();
        long versionBefore = disabled.getVersion();
        LocalDateTime updatedAtBefore = disabled.getUpdatedAt();

        var body = mockMvc.perform(patch(
                                ENABLE_URL,
                                companyId,
                                companyCountryId,
                                regionId,
    ...
    void postAnswersTimestampsAndVersion() throws Exception {
        // The zone's location code is unique per zone, and the row outlives the method, so the new
        // code must differ run to run.
        String uniqueCode = "V" + UUID.randomUUID().toString().replace("-", "").substring(0, 9);
        var body = mockMvc.perform(
                        post(BASE_URL, companyId, companyCountryId, regionId, zoneId, storeId, areaId, storeZoneId)
                                .with(jwt().authorities(ROLE_LC_ADMIN))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(new CreateStoreLocationRequest(
                                        uniqueCode, "Ubicación Nueva Con Versión", null, null))))
