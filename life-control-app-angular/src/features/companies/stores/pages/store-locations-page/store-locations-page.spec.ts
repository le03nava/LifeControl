...
describe('StoreLocationsPage', () => {
  ...
  describe('cascade load failures', () => {
    ...
    it('should surface a store-zones failure instead of an empty list', async () => {
      const storeZoneService = TestBed.inject(StoreZoneService) as unknown as MockStoreZoneService;
      storeZoneService.getStoreZones.mockReturnValue(
        throwError(() => httpFailure('Zonas de tienda caídas')),
      );
      ...
      expect(component.storeZones()).toEqual([]);
      expect(component.storeZonesError()).toBe('Zonas de tienda caídas');
      expect(fixture.nativeElement.querySelector('.cascade-error')?.textContent).toContain(
        'Zonas de tienda caídas',
      );
    });
  });
