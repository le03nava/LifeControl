...
describe('StoreAreasPage', () => {
  ...
  describe('onToggleArea', () => {
    ...
    it('should render the visible "Reactivar" / "Deshabilitar" labels for a mixed list', async () => {
      ...
      const actions = fixture.nativeElement.querySelectorAll('.area-card mat-card-actions');
    ...
    it('should not duplicate the visible button text in an aria-label', async () => {
      ...
      expect(
        fixture.nativeElement.querySelectorAll('.area-card mat-card-actions button[aria-label]')
          .length,
      ).toBe(0);
    });

    it('should warn that disabling also disables the descendant zones and locations', async () => {
      await selectStore();
        ...
        expect.objectContaining({
          data: expect.objectContaining({
            message: expect.stringContaining(
              'También se deshabilitarán todas sus zonas y ubicaciones habilitadas.',
            ),
          }),
        }),
      );
    ...
    it('should notify a successful disable', async () => {
      ...
      const areaService = pageService(StoreAreaService) as unknown as MockStoreAreaService;
      dialogMock.open.mockReturnValue({ afterClosed: () => of(true) });
      areaService.removeArea.mockReturnValue(of(undefined));

      await selectStore();
      component.onToggleArea(mockAreas[0]);
      await settle();

      expect(notifications.showSuccess).toHaveBeenCalledWith('Área deshabilitada correctamente.');
    });
    ...
    it('should notify a successful re-enable', async () => {
      ...
      expect(notifications.showSuccess).toHaveBeenCalledWith('Área reactivada correctamente.');
    });
  ...
  describe('leaf list region', () => {
    ...
    it('should not be busy once the list has loaded', async () => {
      ...
      const grid = fixture.nativeElement.querySelector('.areas-grid');
    ...
    it('should be busy while the list is loading', async () => {
      ...
      areaService.getAreas.mockReturnValue(new Subject<StoreArea[]>().asObservable());
  ...
  describe('role gating', () => {
    describe('read-only user (lc-company-store-read)', () => {
      ...
      it('should not render any per-card write control', async () => {
        ...
        const actions = fixture.nativeElement.querySelector('.area-card mat-card-actions');
        expect(actions).toBeTruthy();
        expect(actions.querySelectorAll('button').length).toBe(0);
      });

      it('should still render the area cards themselves', async () => {
        await selectStore();

        expect(fixture.nativeElement.querySelectorAll('.area-card').length).toBe(mockAreas.length);
      });
    });
