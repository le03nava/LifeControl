...
describe('CompaniesAdminComponent', () => {
  ...
  describe('admin user (lc-admin)', () => {
    ...
    it('should render all 8 cards enabled', () => {
      ...
      expect(dashboardCards).toHaveLength(8);
    });
    ...
    it('should render active cards with "Manage" call-to-action text', () => {
      ...
      expect(actionElements).toHaveLength(8);
  ...
  describe('region-only user (lc-company-region)', () => {
    ...
    it('should enable Regions, Zones, Stores, Store Areas, Store Zones, Store Locations and disable Companies, Countries', () => {
      const cards = component.cards();
    ...
    it('should render 6 cards in the DOM', () => {
      const dashboardCards = fixture.debugElement.queryAll(By.css('.dashboard-card'));
  ...
  describe('zone-only user (lc-company-zone)', () => {
    ...
    it('should enable Zones, Stores, Store Areas, Store Zones, Store Locations and disable Companies, Countries, Regions', () => {
      const cards = component.cards();
      const titles = cards.map((c) => c.title);

      expect(titles).toEqual(['Zones', 'Stores', 'Store Areas', 'Store Zones', 'Store Locations']);
  ...
  describe('store-only user (lc-company-store)', () => {
    ...
    it('should enable only Stores, Store Areas, Store Zones, Store Locations and disable all others', () => {
      const cards = component.cards();
      const titles = cards.map((c) => c.title);

      expect(titles).toEqual(['Stores', 'Store Areas', 'Store Zones', 'Store Locations']);
    ...
    it('should render 4 cards in the DOM', () => {
      const dashboardCards = fixture.debugElement.queryAll(By.css('.dashboard-card'));
  ...
  describe('basic structure', () => {
    ...
    it('should render the page header with title and subtitle', () => {
      ...
      expect(subtitle.textContent.trim()).toBe(
        'Manage companies, countries, regions, zones, stores, store areas, store zones, and store locations',
      );
    });
    ...
    it('should render card icons with correct Material icon names', () => {
      ...
      expect(iconNames).toContain('shelves');
    });
  });
});
